...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_video') {
    header('Content-Type: application/json');
    $title = trim($_POST['title'] ?? '');
    $link = trim($_POST['link'] ?? '');
    $desc = trim($_POST['description'] ?? '');
    $views = isset($_POST['views']) ? (int)$_POST['views'] : 0;
    if ($title === '' || $link === '' || $desc === '') { echo json_encode(['ok' => false, 'error' => 'Please fill required fields']); exit; }
    if ($stmt = $conn->prepare('INSERT INTO watch_demo_list (title, description, link, views) VALUES (?, ?, ?, ?)')) {
        $stmt->bind_param('sssi', $title, $desc, $link, $views);
        $ok = $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_video') {
    ...
    $desc = trim($_POST['description'] ?? '');
    $views = isset($_POST['views']) ? (int)$_POST['views'] : 0;
    if ($id <= 0 || $title === '' || $link === '' || $desc === '') { echo json_encode(['ok' => false, 'error' => 'Missing parameters']); exit; }
    if ($stmt = $conn->prepare('UPDATE watch_demo_list SET title = ?, description = ?, link = ?, views = ? WHERE id = ?')) {
        $stmt->bind_param('sssii', $title, $desc, $link, $views, $id);
        $ok = $stmt->execute();
        $stmt->close();
...
<head>
    ...
    <title>Student Success Office - Demo Video Management</title>
...
<body>
    ...
    $videos = [];
    if ($res = $conn->query('SELECT id, title, description, link, views FROM watch_demo_list ORDER BY id DESC')) { while ($row = $res->fetch_assoc()) { $videos[] = $row; } }
    ?>
        ...
        <main class="main-content">
            <header class="header">
                <div class="header__left">
                    ...
                    <p class="header__subtitle">Manage watch demo links</p>
                </div>
            </header>
            ...
            <section class="section active" style="margin: 0 20px;">
                <div class="table-container">
                    <div class="table-container__header">
                        <h2 class="table-container__title">Watch Demo List</h2>
                        <p class="table-container__subtitle">Create and update demo videos</p>
                    </div>
                    ...
                    <div class="filtration_container">
                        <form method="GET" action="" style="display:flex; width:100%; justify-content:space-between; gap:12px; align-items:center;">
                            <div class="search_input_container" style="flex:1;">
                                ...
                                <input type="text" id="searchQuery" name="q" value="<?php echo htmlspecialchars($q); ?>" placeholder="Search demo videos..." aria-label="Search demo videos">
                            </div>
                            <div class="search_button_container" style="flex:0 0 auto; display:flex; gap:8px; align-items:center;">
                                <button class="button export" id="addVideoBtn" type="button">Add Video</button>
                            </div>
                        </form>
                    </div>
                        ...
                        <table class="table" id="videosTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sortable" data-column="title">Title
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="sort-icon">
                                            <path d="m3 16 4 4 4-4" />
                                            <path d="M7 20V4" />
                                            <path d="M20 8h-5" />
                                    ...
                                    <th class="sortable" data-column="link">Link
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="sort-icon">
                                            <path d="m3 16 4 4 4-4" />
                                            <path d="M7 20V4" />
                                            <path d="M20 8h-5" />
                                    ...
                                    <th class="sortable" data-column="views">Views
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="sort-icon">
                                            <path d="m3 16 4 4 4-4" />
                                            <path d="M7 20V4" />
                                            <path d="M20 8h-5" />
                            ...
                            <tbody>
                                <?php if (empty($videos)): ?>
                                    <tr>
                                        <td colspan="6" style="text-align:center;">No demo videos found.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($videos as $v): ?>
                                        ...
                                        <tr>
                                            <td><?php echo $id; ?></td>
                                            <td data-key="title"><?php echo htmlspecialchars($title !== '' ? $title : 'N/A'); ?></td>
                                            <td data-key="desc"><?php echo htmlspecialchars($desc !== '' ? $desc : 'N/A'); ?></td>
                                            <td data-key="link"><?php echo $link !== '' ? '<a href="'.htmlspecialchars($link).'" target="_blank" rel="noopener noreferrer">'.htmlspecialchars($link).'</a>' : 'N/A'; ?></td>
                                            <td data-key="views"><?php echo $views; ?></td>
                                            <td>
                                                <div style="display:flex; gap:8px; align-items:center;">
                                                    <button type="button" class="button btn-status btn-update"
                                                        ...
                                                        data-views="<?php echo $views; ?>">Update</button>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                    ...
                    <div id="newVideoModal" style="display:none; position: fixed; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.4); z-index: 3500; align-items: center; justify-content: center; backdrop-filter: blur(4px); overflow-y: auto; padding: 16px;">
                        <div style="background: var(--color-card); border-radius: 20px; text-align: center; max-width: 520px; width: 92%; box-shadow: 0 20px 60px rgba(0,0,0,0.1), 0 8px 25px rgba(0,0,0,0.08); margin: auto; max-height: 85vh; overflow-y: auto; border: 1px solid var(--color-border); color: var(--color-text);">
                            <div style="padding: 32px 32px 16px 32px;">
                                ...
                                <h2 style="margin: 0 0 8px; font-size: 1.4rem;">Add Video</h2>
                                <p style="color: #718096; margin: 0; line-height: 1.5; font-size: 0.95rem;">Enter video details.</p>
                            </div>
                            <form id="newVideoForm" style="padding: 0 32px 24px 32px; text-align:left;">
                                <div style="display:grid; grid-template-columns: 1fr; gap:12px;">
                                    <div>
                                        <label style="display:block; font-weight:600; margin-bottom:6px;">Title <span style="color:#ef4444;">*</span></label>
                                    ...
                                    <div>
                                        <label style="display:block; font-weight:600; margin-bottom:6px;">Link <span style="color:#ef4444;">*</span></label>
                                        <input name="link" type="url" required style="width:100%; padding: 12px; border: 2px solid var(--color-border); border-radius: 10px; background: var(--color-card); color: var(--color-text);">
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <label style="display:block; font-weight:600; margin-bottom:6px;">Views</label>
                                        <input name="views" type="number" min="0" value="0" style="width:100%; padding: 12px; border: 2px solid var(--color-border); border-radius: 10px; background: var(--color-card); color: var(--color-text);">
                                    </div>
                                </div>
                                <div style="padding: 20px 0 0; display:flex; gap:12px;">
                                    <button type="button" id="cancelNewVideoBtn" style="flex:1; padding: 14px 24px; border: 2px solid var(--color-border); background: var(--color-card); color: var(--color-text); border-radius: 12px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s ease;">Cancel</button>
                                    <button type="button" id="confirmNewVideoBtn" style="flex:1; padding: 14px 24px; border: none; background: linear-gradient(135deg, #18a558 0%, #136515 100%); color: white; border-radius: 12px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s ease;">Add Video</button>
                                </div>
                            </form>
                        </div>
                    </div>

                    <div id="updateVideoModal" style="display:none; position: fixed; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.4); z-index: 3500; align-items: center; justify-content: center; backdrop-filter: blur(4px); overflow-y: auto; padding: 16px;">
                        <div style="background: var(--color-card); border-radius: 20px; text-align: center; max-width: 520px; width: 92%; box-shadow: 0 20px 60px rgba(0,0,0,0.1), 0 8px 25px rgba(0,0,0,0.08); margin: auto; max-height: 85vh; overflow-y: auto; border: 1px solid var(--color-border); color: var(--color-text);">
                            <div style="padding: 32px 32px 16px 32px;">
                                <div style="width: 56px; height: 56px; background: linear-gradient(135deg, #18a558 0%, #136515 100%); border-radius: 16px; margin: 0 auto 20px auto; display: flex; align-items: center; justify-content: center;">
                                    <svg style="width: 28px; height: 28px; color: white;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <h2 style="margin: 0 0 8px; font-size: 1.4rem;">Update Video</h2>
                                <p style="color: #718096; margin: 0; line-height: 1.5; font-size: 0.95rem;">Edit video details.</p>
                            </div>
                            <form id="updateVideoForm" style="padding: 0 32px 24px 32px; text-align:left;">
                                ...
                                <div style="padding: 20px 0 0; display:flex; gap:12px;">
                                    <button type="button" id="cancelUpdateVideoBtn" style="flex:1; padding: 14px 24px; border: 2px solid var(--color-border); background: var(--color-card); color: var(--color-text); border-radius: 12px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s ease;">Cancel</button>
                                    <button type="button" id="confirmUpdateVideoBtn" style="flex:1; padding: 14px 24px; border: none; background: linear-gradient(135deg, #18a558 0%, #136515 100%); color: white; border-radius: 12px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s ease;">Update Video</button>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            headers.forEach(h => {
                h.addEventListener('click', function() {
                    ...
                    rows.sort(function(r1, r2) { var v1 = getCellValue(r1, key), v2 = getCellValue(r2, key); if (key === 'id' || key === 'views') { return dir === 'asc' ? compareNumeric(v1, v2) : compareNumeric(v2, v1); } return dir === 'asc' ? compareText(v1, v2) : compareText(v2, v1); });
            ...
            function filterRows(q) {
                ...
                if (!noRow) { noRow = document.createElement('tr'); noRow.id = 'noResultsRow'; var td = document.createElement('td'); var colCount = table.querySelectorAll('thead th').length; td.colSpan = colCount; td.style.textAlign = 'center'; td.style.color = 'var(--color-text-light)'; td.textContent = 'No matching videos.'; noRow.appendChild(td); tbody.appendChild(noRow); }
                noRow.style.display = visible === 0 ? '' : 'none';
            }
            if (searchInput) { filterRows(searchInput.value); searchInput.addEventListener('input', function() { filterRows(this.value); if (window.videosPagination) { window.videosPagination.updateFilteredRows(); window.videosPagination.currentPage = 1; window.videosPagination.updateDisplay(); } }); }

            class VideosPagination {
                constructor() { this.table = document.getElementById('videosTable'); this.tbody = this.table?.querySelector('tbody'); this.select = document.getElementById('rowsPerPageSelect'); this.info = document.getElementById('paginationInfo'); this.prev = document.getElementById('prevPage'); this.next = document.getElementById('nextPage'); this.currentPage = 1; this.rowsPerPage = this.getRowsPerPage(); this.allRows = []; this.filteredRows = []; this.init(); }
            ...
            window.videosPagination = new VideosPagination();
            ...
            var newModal = document.getElementById('newVideoModal');
            var cancelNewBtn = document.getElementById('cancelNewVideoBtn');
            var confirmNewBtn = document.getElementById('confirmNewVideoBtn');
            var updateModal = document.getElementById('updateVideoModal');
            var cancelUpdateBtn = document.getElementById('cancelUpdateVideoBtn');
            var confirmUpdateBtn = document.getElementById('confirmUpdateVideoBtn');
            ...
            confirmNewBtn?.addEventListener('click', async function(){
                var form = document.getElementById('newVideoForm'); var fd = new FormData(form);
                var title = String(fd.get('title')||'').trim(); var link = String(fd.get('link')||'').trim(); var desc = String(fd.get('description')||'').trim(); var views = Number(fd.get('views')||0);
                if (!title || !link || !desc) return;
                try{
                    window.showLoader && window.showLoader();
                    var resp = await fetch('demo_video_management.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ action:'create_video', title:title, link:link, description:desc, views:String(views) }).toString() });
                    var data = await resp.json();
                    window.hideLoader && window.hideLoader();
                    if (data && data.ok){ newModal.style.display='none'; showMessage('Video Added','New demo video created successfully.','success', function(){ location.reload(); }); }
                    else { newModal.style.display='none'; showMessage('Create Failed', (data && data.error) ? String(data.error) : 'Failed to add video','error'); }
                }catch(err){ window.hideLoader && window.hideLoader(); newModal.style.display='none'; showMessage('Error','Error adding video','error'); }
            });

            tbody?.addEventListener('click', function(e){ var ubtn = e.target.closest('.btn-update'); if (!ubtn) return; if (updateModal) { updateModal.style.display='flex'; var form = document.getElementById('updateVideoForm'); form.querySelector('[name="id"]').value = ubtn.dataset.id || ''; form.querySelector('[name="title"]').value = ubtn.dataset.title || ''; form.querySelector('[name="description"]').value = ubtn.dataset.desc || ''; form.querySelector('[name="link"]').value = ubtn.dataset.link || ''; form.querySelector('[name="views"]').value = ubtn.dataset.views || '0'; } });
            ...
            confirmUpdateBtn?.addEventListener('click', async function(){
                var form = document.getElementById('updateVideoForm'); var fd = new FormData(form);
                var id = Number(fd.get('id')||0); var title = String(fd.get('title')||'').trim(); var link = String(fd.get('link')||'').trim(); var desc = String(fd.get('description')||'').trim(); var views = Number(fd.get('views')||0);
                if (!id || !title || !link || !desc) return;
                try{
                    window.showLoader && window.showLoader();
                    var resp = await fetch('demo_video_management.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ action:'update_video', id:String(id), title:title, link:link, description:desc, views:String(views) }).toString() });
                    var data = await resp.json();
                    window.hideLoader && window.hideLoader();
                    if (data && data.ok){
                        ...
                        if (row){
                            row.children[getColumnIndex('title')].textContent = title || 'N/A';
                            row.children[getColumnIndex('desc')].textContent = desc || 'N/A';
                            var linkCell = row.children[getColumnIndex('link')];
                            if (linkCell){ linkCell.innerHTML = link ? `<a href="${link}" target="_blank" rel="noopener noreferrer">${link}</a>` : 'N/A'; }
                            row.children[getColumnIndex('views')].textContent = String(views || 0);
                        }
                        showMessage('Video Updated','Changes saved successfully.','success');
                    } else {
                        updateModal.style.display='none'; showMessage('Update Failed', (data && data.error) ? String(data.error) : 'Failed to update video','error');
                    }
                }catch(err){ window.hideLoader && window.hideLoader(); updateModal.style.display='none'; showMessage('Error','Error updating video','error'); }
            });
        });
    </script>
</body>
